      ...
      <div class="filter-select-box">
        <button :class="{ active: filterMenu }" class="filter-select" @click="filterMenu = !filterMenu">
          <div class="select-value">
            {{ activeCategory !== 0 ? (locale === 'en' ? activeCategoryName?.en : activeCategoryName?.ru_RU) : 'Select Category' }}
          </div>
      ...
      <ul class="project-list">
        <li v-for="project in projectList" :key="project.id" :class="{ active: activeCategory === project.category.id || activeCategory === 0 }" class="project-item">
          <a class="cursor-pointer" @click="showItem(project.id)">
            ...
            <p class="project-category">{{ locale === 'en' ? project.category.title?.en : project.category.title?.ru_RU }}</p>
        ...
        <section class="testimonials-modal block">
          ...
          <div class="modal-content space-y-3 mt-4">
            ...
            <small class="flex items-center justify-start gap-2 text-gray-500">
              <Icon icon="foundation:calendar" /><span>{{ locale === 'en' ? activeItem.category?.date?.en : activeItem.category?.date?.ru_RU }}</span> | <Icon icon="dashicons:category" /><span>{{ locale === 'en' ? activeItem.category?.title?.en : activeItem.category?.title?.ru_RU }}</span> | <Icon icon="ri:search-eye-line" /><a :href="activeItem.url" target="_blank">{{ locale === 'en' ? activeItem.desc?.en : activeItem.desc?.ru_RU }}</a>
            </small>

            <p class="text-justify" v-html="locale === 'en' ? activeItem.content?.en : activeItem.content?.ru_RU" />
          </div>
        </section>
      </div>
    </section>
